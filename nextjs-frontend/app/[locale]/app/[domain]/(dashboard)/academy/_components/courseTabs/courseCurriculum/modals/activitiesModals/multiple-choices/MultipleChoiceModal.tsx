...
import EditCompleteCondition from './EditCompleteCondition'
...
const MultipleChoiceModal = ({ step = 1 }: ModalProps) => {
    const [isLoading, setIsLoading] = useState(false);
    const [currentStep, setCurrentStep] = useState(step);

    const { nbrQuestionsState, difficultyState } = useMultiChoiceActivityStore()
            ...
            <Dialog as='div' onClose={() => { setModalOpenState(false) }}>
                ...
                >
                    <div className="max-w-5xl mx-auto h-full flex items-center w-full md:w-auto px-5 md:px-0">
                        <Dialog.Panel className={`relative h-[600px] w-full md:w-[850px] overflow-y-scroll no-scrollbar rounded-3xl shadow-2xl aspect-auto bg-[#F4F7FE] overflow-hidden px-10 pt-5 pb-8`}>
                            {showHeading() &&
                                <Dialog.Title
                                    as="h3"
                            ...
                            <div className='z-10 text-center cursor-pointer p-1 absolute top-4 right-5 rounded-md bg-[#D9DADD] hover:bg-[#D9DADD]/80'>
                                <X
                                    size={16}
                            ...
                            {
                                {
                                    ...
                                    2: <StepTwo
                                        setCurrentStep={setCurrentStep}
                                        nbrQuestions={nbrQuestionsState}
                                        difficulty={difficultyState}
                                    />,
                                    3: <EditCompleteCondition
                                        setCurrentStep={setCurrentStep}
                                    />
                                }[currentStep]
                            }
