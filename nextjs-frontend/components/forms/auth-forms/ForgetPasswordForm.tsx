...
const ForgetPasswordForm = ({ type, loginUrl, currentWorkspace }: { type: string, loginUrl: string, currentWorkspace: string }) => {
    ...
    const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            const response = await FetchForgetPassword({email, subdomain: currentWorkspace}, type);
            if (response) {
                if (response.status === 422) {
                    toast({ variant: "destructive", description: "Please Enter Valid Email" });
    ...
    return (
        <div className={`${flexCenter} sm:min-w-[300px] md:min-w-[400px]`}>
            ...
            {isSuccess ?
                <AlertBox type="success" message="Please check your email inbox for a link to complete the reset." /> :
                <>
                    <div className={`${flexCenter} mb-5 md:mb-7.5 max-w-[400px]`}>
                        <p className='text-center text-lg md:text-2xl font-medium leading-[20px] md:leading-[29px]'>Reset your password</p>
                    ...
                    >
                        ...
                        <Link
                            type="button"
                            href={loginUrl}
                            className="text-center w-full transform rounded-lg shadow-lg bg-[#8781E8] px-6 py-3 mb-5 text-sm font-medium tracking-wide text-white transition-colors duration-300 hover:opacity-90 focus:outline-none">Back to Login</Link>
                    </form>
                </>
            }
